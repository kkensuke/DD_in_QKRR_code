...
class SyntheticQuantumDataset:
    ...
    def generate_true_params(self) -> np.ndarray:
        """Draw w_* uniformly from the unit sphere in R^(4^n)."""
        dim = 4 ** self.N_QUBITS
        params = self.rng.normal(0, 1, size=(dim,))
        params /= np.linalg.norm(params)
        return params

    def _teacher_observable(self, true_params) -> np.ndarray:
        """Return O_* = sum_a w_{*,a} P_a for the normalized Pauli basis."""
        expected_dim = 4 ** self.N_QUBITS
        true_params = np.asarray(true_params, dtype=float)
        if true_params.shape != (expected_dim,):
            raise ValueError(
                f"true_params must have shape ({expected_dim},), got {true_params.shape}"
            )
        ...
        for weight, pauli_indices in zip(
            true_params, product(range(4), repeat=self.N_QUBITS)
        ):
            pauli_string = np.array([[1.0]], dtype=complex)
            for pauli_index in pauli_indices:
                pauli_string = np.kron(
                    pauli_string, pauli_matrices[pauli_index]
                )
            observable += weight * pauli_string

        # Tr(P_a P_b) = delta_ab for P_a = sigma_a / sqrt(2^n).
        return observable / np.sqrt(2 ** self.N_QUBITS)

    def true_function(self, X, true_params) -> np.ndarray:
        """Evaluate f_*(u) = w_*^T r(u) = <psi(u)|O_*|psi(u)>."""
        observable = self._teacher_observable(true_params)
        states = np.asarray(
            [self.quantum_kernel.state_vector(x) for x in X], dtype=complex
        )
        values = np.einsum(
            "bi,ij,bj->b", states.conj(), observable, states, optimize=True
        )
        return np.real_if_close(values).real

    def generate_samples(self, sample_size, true_params):
        """Generate an i.i.d. input sample, noisy labels, and noise-free targets."""
        X = self.rng.normal(0, 1, size=(sample_size, self.x_dim))
        f_true = self.true_function(X, true_params)
        y = f_true + self.noise_sigma * self.rng.normal(0, 1, size=sample_size)
        return X, y, f_true

    def generate_dataset(self, true_params=None):
        self.true_params = (
            true_params if true_params is not None else self.generate_true_params()
        )
        sample_size = self.train_size + self.test_size
        X, y, _ = self.generate_samples(sample_size, self.true_params)
...
class TheoreticalTestRisk:
    ...
    def K_eigen_and_beta(self, X_est, target_values):
        ...
        if X_est.shape[0] != target_values.shape[0]:
            raise ValueError("X_est and target_values must have the same sample size")

        K_est = self.quantum_kernel.kernel_matrix(X_est, X_est, same=True)
        N_est = target_values.shape[0]
        p = 3 ** self.N_QUBITS if self.TPA else 4 ** self.N_QUBITS
        if N_est < p:
            raise ValueError(
                f"N_est={N_est} must be at least p={p} to estimate all p modes"
            )
